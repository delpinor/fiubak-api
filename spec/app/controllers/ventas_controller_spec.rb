...
describe 'Ventas controller' do
  ...
  it 'La respuesta debe ser un mensaje exitoso' do
    ...
    post('/usuarios/343/intenciones_de_venta', datos_auto.to_json, header_con_token)
  ...
  it 'Al intentar registrar un auto sin registrar el usuario recibo un mensaje de error' do
    post('/usuarios/1/intenciones_de_venta', datos_auto.to_json, header_con_token)
    body = JSON.parse(last_response.body)
  ...
  it 'cuando creamos 2 intenciones de venta la respuesta es exitosa' do
    post('/usuarios', datos_usuario_2.to_json, header_con_token)
    post('/usuarios', datos_usuario.to_json, header_con_token)
    post('/usuarios/343/intenciones_de_venta', datos_auto.to_json, header_con_token)
    post('/usuarios/400/intenciones_de_venta', datos_auto_2.to_json, header_con_token)
    body = JSON.parse(last_response.body)
    id_venta = body['id']
    expect(body['mensaje']).to eq("Intención de venta registrada bajo el nro. #{id_venta}")
    expect(body['id']).to be_present
  ...
  context 'Consulta de intenciones de venta' do
    ...
    it 'Cuando creo una intencion de venta y consulto las intenciones de venta del usuario entonces recibo una intencion de venta con su estado y id' do
      post('/usuarios', datos_usuario_3.to_json, header_con_token)
      post('/usuarios/1234/intenciones_de_venta', datos_auto.to_json, header_con_token)
      id_intencion_de_venta = JSON.parse(last_response.body)['id']
      get('/intenciones_de_venta/' + id_intencion_de_venta.to_s, datos_auto.to_json, header_con_token)
      body = JSON.parse(last_response.body)
